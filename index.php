...
if(preg_match('#^/auftrag/(\d{8})$#',$path,$m)&&$method==='GET'){
 $s=require_seller();$st=db()->prepare("SELECT o.*,f.title,f.description,c.name category_name FROM orders o JOIN offers f ON f.id=o.offer_id JOIN categories c ON c.id=f.category_id WHERE o.order_no=? AND o.seller_id=?");$st->execute([$m[1],$s['id']]);$o=$st->fetch();if(!$o)not_found();
