...
if($path==='/'&&$method==='GET'){
 $offers=db()->query("SELECT o.*,c.name category_name FROM offers o JOIN categories c ON c.id=o.category_id WHERE o.status='active' ORDER BY o.created_at DESC LIMIT 6")->fetchAll();
 ob_start();?>
...
if($path==='/angebote'&&$method==='GET'){
 ...
 $sql="SELECT o.*,c.name category_name FROM offers o JOIN categories c ON c.id=o.category_id WHERE o.status='active'";$args=[];
...
if($path==='/dashboard'&&$method==='GET'){
 ...
 ob_start();?><div class="dashboard-head"><div><div class="eyebrow">Heute</div><h1>Hallo <?=e($s['first_name'])?></h1><p class="meta"><?=$s['email_verified_at']?'E-Mail bestätigt':'E-Mail noch nicht bestätigt – Angebote können noch nicht angenommen werden.'?></p></div><a class="btn" href="<?=e(url('/angebote'))?>">Angebote entdecken</a></div><div class="grid"><div class="card"><div class="meta">Vorgemerkt</div><div class="stat"><?=money($wallet['reserved']??0)?></div></div><div class="card"><div class="meta">Verfügbar</div><div class="stat"><?=money($wallet['available']??0)?></div></div><div class="card"><div class="meta">Aktive Aufträge</div><div class="stat"><?=count(array_filter($orders,fn($o)=>!in_array($o['status'],['completed','rejected','archived'],true)))?></div></div></div><h2>Deine Aufträge</h2><div class="table-wrap"><table><thead><tr><th>Nr.</th><th>Auftrag</th><th>Status</th><th>Wert</th><th></th></tr></thead><tbody><?php foreach($orders as $o):?><tr><td><?=e($o['order_no'])?></td><td><?=e($o['title'])?></td><td><span class="badge"><?=e($o['status'])?></span></td><td><?=money($o['total_compensation'])?></td><td><a href="<?=e(url('/auftrag/'.$o['order_no']))?>">Öffnen</a></td></tr><?php endforeach;?></tbody></table></div><?php render('Dashboard',ob_get_clean());exit;
}
if(preg_match('#^/auftrag/(\d{8})$#',$path,$m)&&$method==='GET'){
 $s=require_seller();$st=db()->prepare("SELECT o.*,f.title,f.description,f.evidence_rules_json,f.fulfillment_type,c.name category_name FROM orders o JOIN offers f ON f.id=o.offer_id JOIN categories c ON c.id=f.category_id WHERE o.order_no=? AND o.seller_id=?");$st->execute([$m[1],$s['id']]);$o=$st->fetch();if(!$o)not_found();
 ...
 $preAccepted=count(array_filter($evidences,fn($x)=>$x['evidence_type']==='precheck'&&$x['status']==='accepted'));
 $prePending=count(array_filter($evidences,fn($x)=>$x['evidence_type']==='precheck'&&$x['status']==='submitted'));
 ob_start();?><div class="dashboard-head"><div><div class="eyebrow">Auftrag <?=e($o['order_no'])?></div><h1><?=e($o['title'])?></h1><span class="badge"><?=e($o['status'])?></span></div><div class="price"><?=money($o['total_compensation'])?></div></div><div class="grid two"><section class="panel"><h2>Ablauf</h2><div class="timeline"><div>1. Vorbereitung / Vorabkontrolle <?= $o['status']==='precheck'?'← aktuell':''?></div><div>2. Durchführung</div><div>3. Versand / digitale Abgabe</div><div>4. Prüfung</div><div>5. Auszahlung</div><div>6. Archiv</div></div></section><section class="panel"><h2>Vorabkontrolle</h2><?php if($o['status']==='precheck'):?><p>Lade die für den konkreten Artikel erforderlichen Startnachweise direkt über die Kamera hoch.</p><p><strong><?=e($preAccepted)?> / <?=e($preRequired)?></strong> Pflichtnachweise freigegeben<?php if($prePending):?> · <?=e($prePending)?> warten auf Prüfung<?php endif;?></p><div class="progress"><span style="width:<?=e((string)min(100,round(($preAccepted/$preRequired)*100)))?>%"></span></div><br><form method="post" action="<?=e(url('/auftrag/'.$o['order_no'].'/nachweis'))?>" enctype="multipart/form-data"><?=csrf_field()?><input type="hidden" name="type" value="precheck"><label>Kurze Artikelbezeichnung<input name="item_label" required placeholder="z. B. schwarze Sportsocken"></label><label>Pflichtfoto<input data-camera-input type="file" name="evidence" required></label><button class="btn">Nachweis einreichen</button></form><?php else:?><p class="meta">Die Vorabkontrolle ist abgeschlossen bzw. befindet sich nicht mehr in der Vorbereitungsphase.</p><?php endif;?></section></div><h2>Nachweise</h2><div class="table-wrap"><table><thead><tr><th>Typ</th><th>Zeitpunkt</th><th>Status</th></tr></thead><tbody><?php foreach($evidences as $evd):?><tr><td><?=e($evd['evidence_type'])?></td><td><?=e(date('d.m.Y H:i',strtotime($evd['created_at'])))?></td><td><?=e($evd['status'])?></td></tr><?php endforeach;?></tbody></table></div><div class="grid two" style="margin-top:18px">
<section class="panel"><h2>Während der Durchführung</h2><?php if($o['status']==='running'):?><form method="post" action="<?=e(url('/auftrag/'.$o['order_no'].'/tagesnachweis'))?>" enctype="multipart/form-data"><?=csrf_field()?><div class="form-grid"><label>Tag<input type="number" min="1" name="day_no" value="1" required></label><label>Zeitfenster<select name="window_key"><option value="morning">Morgen</option><option value="midday">Mittag</option><option value="evening">Abend</option></select></label></div><label>Live-Nachweis<input data-camera-input type="file" name="evidence" required></label><button class="btn">Tagesnachweis einreichen</button></form><?php else:?><p class="meta">Tagesnachweise werden freigeschaltet, sobald der Auftrag läuft.</p><?php endif;?></section>
<section class="panel"><h2>Auftrag & Kommunikation</h2><div class="actions"><a class="btn secondary" href="<?=e(url('/auftrag/'.$o['order_no'].'/chat'))?>">Auftragschat</a><a class="btn secondary" href="<?=e(url('/auftrag/'.$o['order_no'].'/versand'))?>">Versand</a><a class="btn secondary" href="<?=e(url('/auftrag/'.$o['order_no'].'/digital'))?>">Digitale Abgabe</a></div><?php if($o['status']==='running'):?><hr><h3>Beschädigung melden</h3><form method="post" action="<?=e(url('/auftrag/'.$o['order_no'].'/beschaedigung'))?>" enctype="multipart/form-data"><?=csrf_field()?><label>Beschreibung<textarea name="reason" required></textarea></label><label>Nachweisfoto<input data-camera-input type="file" name="evidence"></label><button class="btn secondary">Beschädigung melden</button></form><?php endif;?></section>
...
if(preg_match('#^/auftrag/(\d{8})/nachweis$#',$path,$m)&&$method==='POST'){
 ...
 try{$up=private_upload($_FILES['evidence']??[],'order-'.$o['id']);$run=db()->prepare("SELECT id FROM order_runs WHERE order_id=? ORDER BY run_no DESC LIMIT 1");$run->execute([$o['id']]);$runId=$run->fetchColumn()?:null;
  if(post('type')==='precheck'&&post('item_label')!==''){ $cnt=db()->prepare("SELECT COUNT(*) FROM order_items WHERE order_id=?");$cnt->execute([$o['id']]);if(!(int)$cnt->fetchColumn())db()->prepare("INSERT INTO order_items(order_id,order_run_id,label,locked_at) VALUES(?,?,?,NOW())")->execute([$o['id'],$runId,post('item_label')]);}
  db()->prepare("INSERT INTO evidences(order_id,order_run_id,seller_id,evidence_type,file_path,mime_type,file_size,sha256) VALUES(?,?,?,?,?,?,?,?)")->execute([$o['id'],$runId,$s['id'],post('type','precheck'),$up['path'],$up['mime'],$up['size'],$up['sha256']]);flash('success','Nachweis wurde sicher gespeichert.');
...
if($path==='/admin'&&$method==='GET'){
 ...
 ob_start();?><div class="dashboard-head"><div><div class="eyebrow">Administration</div><h1>Heute</h1></div><div class="actions"><a class="btn" href="<?=e(url('/admin/angebote'))?>">Angebote</a><a class="btn secondary" href="<?=e(url('/admin/kategorien'))?>">Kategorien</a><a class="btn secondary" href="<?=e(url('/admin/auftraege'))?>">Aufträge</a></div></div><div class="grid"><div class="card"><div class="meta">Aktive Angebote</div><div class="stat"><?=$counts['offers']?></div></div><div class="card"><div class="meta">Verkäuferinnen</div><div class="stat"><?=$counts['sellers']?></div></div><div class="card"><div class="meta">Aktive Aufträge</div><div class="stat"><?=$counts['orders']?></div></div></div><h2>Sofort bearbeiten</h2><div class="card"><strong><?=$counts['prechecks']?> offene Vorabnachweise</strong><p class="meta">Noch nicht geprüfte Vorabkontrollen priorisiert bearbeiten.</p><a href="<?=e(url('/admin/auftraege'))?>">Aufträge öffnen →</a></div><?php render('Admin Dashboard',ob_get_clean());exit;
}
if($path==='/admin/kategorien'&&$method==='GET'){
 require_admin();$cats=db()->query("SELECT * FROM categories ORDER BY sort_order,name")->fetchAll();ob_start();?><div class="dashboard-head"><div><div class="eyebrow">Administration</div><h1>Kategorien</h1></div></div><form method="post" class="panel form-grid"><?=csrf_field()?><input name="name" placeholder="Neue Kategorie" required><button class="btn">Kategorie anlegen</button></form><br><div class="table-wrap"><table><thead><tr><th>Name</th><th>System</th><th>Status</th></tr></thead><tbody><?php foreach($cats as $c):?><tr><td><?=e($c['name'])?></td><td><?=$c['is_system']?'Ja':'Nein'?></td><td><?=$c['is_active']?'Aktiv':'Deaktiviert'?></td></tr><?php endforeach;?></tbody></table></div><?php render('Kategorien',ob_get_clean());exit;
}
...
if(preg_match('#^/admin/auftrag/(\d{8})$#',$path,$m)&&$method==='GET'){
 require_admin();$st=db()->prepare("SELECT o.*,f.title,CONCAT(s.first_name,' ',s.last_name) seller_name,s.email FROM orders o JOIN offers f ON f.id=o.offer_id JOIN sellers s ON s.id=o.seller_id WHERE o.order_no=?");$st->execute([$m[1]]);$o=$st->fetch();if(!$o)not_found();$ev=db()->prepare("SELECT * FROM evidences WHERE order_id=? ORDER BY created_at");$ev->execute([$o['id']]);$evidences=$ev->fetchAll();
 ob_start();?><div class="dashboard-head"><div><div class="eyebrow">Auftrag <?=e($o['order_no'])?></div><h1><?=e($o['title'])?></h1><p class="meta"><?=e($o['seller_name'])?> · <?=e($o['email'])?></p></div><span class="badge"><?=e($o['status'])?></span></div><div class="panel"><h2>Nachweise</h2><?php foreach($evidences as $evd):?><div class="timeline"><div><strong><?=e($evd['evidence_type'])?></strong> · <?=e(date('d.m.Y H:i',strtotime($evd['created_at'])))?> · <?=e($evd['status'])?> <?php if($evd['status']==='submitted'):?><div class="actions" style="margin-top:8px"><form method="post" action="<?=e(url('/admin/nachweis/'.$evd['id'].'/freigeben'))?>" style="display:inline"><?=csrf_field()?><button class="btn">Freigeben</button></form><form method="post" action="<?=e(url('/admin/nachweis/'.$evd['id'].'/ablehnen'))?>" style="display:flex;gap:8px;align-items:center"><?=csrf_field()?><input name="reason" placeholder="Ablehnungsgrund" required><button class="btn danger">Beanstanden</button></form></div><?php elseif($evd['status']==='rejected' && $evd['rejection_reason']):?><div class="meta">Grund: <?=e($evd['rejection_reason'])?></div><?php endif;?></div></div><?php endforeach;?><?php if(!$evidences):?><p class="meta">Noch keine Nachweise.</p><?php endif;?></div><?php render('Auftrag '.$o['order_no'],ob_get_clean());exit;
}
