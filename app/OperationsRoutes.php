...
if (preg_match('#^/admin/verkaeuferin/(\d+)$#',$path,$m) && $method==='GET') {
    ...
    $wq=db()->prepare("SELECT * FROM wallet_entries WHERE seller_id=? ORDER BY created_at DESC,id DESC LIMIT 100");$wq->execute([$s['id']]);$walletEntries=$wq->fetchAll();
    $pq=db()->prepare("SELECT * FROM payout_requests WHERE seller_id=? ORDER BY created_at DESC,id DESC LIMIT 100");$pq->execute([$s['id']]);$payouts=$pq->fetchAll();
    $ppq=db()->prepare("SELECT * FROM payout_profiles WHERE seller_id=? LIMIT 1");$ppq->execute([$s['id']]);$payoutProfile=$ppq->fetch()?:null;
    $shq=db()->prepare("SELECT sh.*,o.order_no FROM shipments sh JOIN orders o ON o.id=sh.order_id WHERE o.seller_id=? ORDER BY COALESCE(sh.shipped_at,o.created_at) DESC");$shq->execute([$s['id']]);$shipments=$shq->fetchAll();
    $eq=db()->prepare("SELECT e.id,e.evidence_type,e.status,e.day_no,e.window_key,e.created_at,o.order_no FROM evidences e JOIN orders o ON o.id=e.order_id WHERE o.seller_id=? ORDER BY e.created_at DESC,e.id DESC LIMIT 100");$eq->execute([$s['id']]);$sellerEvidence=$eq->fetchAll();
    $vq=db()->prepare("SELECT v.*,o.order_no FROM violations v JOIN orders o ON o.id=v.order_id WHERE o.seller_id=? ORDER BY v.created_at DESC,v.id DESC LIMIT 100");$vq->execute([$s['id']]);$sellerViolations=$vq->fetchAll();
    $seq=db()->prepare("SELECT * FROM system_events WHERE seller_id=? ORDER BY created_at DESC,id DESC LIMIT 100");$seq->execute([$s['id']]);$sellerEvents=$seq->fetchAll();
    $laq=db()->prepare("SELECT * FROM seller_legal_acceptances WHERE seller_id=? ORDER BY created_at DESC,id DESC");$laq->execute([$s['id']]);$legalAcceptances=$laq->fetchAll();
    ...
    <section class="panel" style="margin-top:18px"><h2>Rechts- und Regelbestätigungen</h2><div class="table-wrap"><table><thead><tr><th>Zeit</th><th>Kontext</th><th>Version</th><th>Bestätigte Bereiche</th></tr></thead><tbody><?php foreach($legalAcceptances as $la): $payload=json_decode($la['payload_json']??'{}',true)?:[];?><tr><td><?=e(date('d.m.Y H:i',strtotime($la['created_at'])))?></td><td><?=e($la['context'])?></td><td><?=e($la['rules_version'])?></td><td><?php foreach($payload as $key=>$value){if($value===true||$value===1||$value==='1')echo '<span class="badge">'.e($key).'</span> ';}?></td></tr><?php endforeach;?><?php if(!$legalAcceptances):?><tr><td colspan="4">Für ältere Konten liegt noch kein versionierter Registrierungs-Snapshot vor.</td></tr><?php endif;?></tbody></table></div></section>

    <h2>Aufträge</h2><div class="table-wrap"><table><thead><tr><th>Nr.</th><th>Auftrag</th><th>Status</th><th>Wert</th><th>Archiv</th><th></th></tr></thead><tbody><?php foreach($orders as $x):?><tr><td><?=e($x['order_no'])?></td><td><?=e($x['title'])?></td><td><?=e($x['status'])?></td><td><?=money($x['total_compensation'])?></td><td><?=$x['archived_at']?'Ja':'Nein'?></td><td><a href="<?=e(url('/admin/auftrag/'.$x['order_no']))?>">Öffnen</a></td></tr><?php endforeach;?></tbody></table></div>

    <div class="grid two" style="margin-top:18px">
      <section class="panel"><h2>Auszahlungsdaten</h2><?php if($payoutProfile):?><p>Kontoinhaber: <strong><?=e($payoutProfile['account_holder']?:'–')?></strong><br>IBAN: <?=e($payoutProfile['iban']?:'–')?><?php if($payoutProfile['bic']):?><br>BIC: <?=e($payoutProfile['bic'])?><?php endif;?><?php if($payoutProfile['paypal']):?><br>PayPal: <?=e($payoutProfile['paypal'])?><?php endif;?></p><?php else:?><p class="meta">Keine Auszahlungsdaten hinterlegt.</p><?php endif;?>
      <h3>Auszahlungsanträge</h3><div class="table-wrap"><table><thead><tr><th>#</th><th>Betrag</th><th>Gebühr</th><th>Netto</th><th>Methode</th><th>Status</th><th>Zeit</th></tr></thead><tbody><?php foreach($payouts as $p):?><tr><td><?=e($p['id'])?></td><td><?=money($p['amount'])?></td><td><?=money($p['fee'])?></td><td><?=money($p['net_amount'])?></td><td><?=e($p['method'])?></td><td><?=e($p['status'])?></td><td><?=e(date('d.m.Y H:i',strtotime($p['created_at'])))?></td></tr><?php endforeach;?><?php if(!$payouts):?><tr><td colspan="7">Keine Auszahlungsanträge.</td></tr><?php endif;?></tbody></table></div></section>

      <section class="panel"><h2>Wallet-Historie</h2><div class="table-wrap"><table><thead><tr><th>Zeit</th><th>Typ</th><th>Betrag</th><th>Auftrag</th><th>Intern</th></tr></thead><tbody><?php foreach($walletEntries as $w):?><tr><td><?=e(date('d.m.Y H:i',strtotime($w['created_at'])))?></td><td><?=e($w['entry_type'])?></td><td><?=money($w['amount'])?></td><td><?=e($w['order_id']?:'–')?></td><td><?=e($w['description']??'')?></td></tr><?php endforeach;?><?php if(!$walletEntries):?><tr><td colspan="5">Keine Wallet-Bewegungen.</td></tr><?php endif;?></tbody></table></div></section>
    </div>

    <h2>Versandvorgänge</h2><div class="table-wrap"><table><thead><tr><th>Auftrag</th><th>Status</th><th>Dienstleister</th><th>Tracking</th><th>Versand</th><th>Eingang</th></tr></thead><tbody><?php foreach($shipments as $sh):?><tr><td><a href="<?=e(url('/admin/auftrag/'.$sh['order_no']))?>"><?=e($sh['order_no'])?></a></td><td><?=e($sh['status'])?></td><td><?=e($sh['carrier']?:'–')?></td><td><?=e($sh['tracking_number']?:'–')?></td><td><?=e($sh['shipped_at']?date('d.m.Y H:i',strtotime($sh['shipped_at'])):'–')?></td><td><?=e($sh['received_at']?date('d.m.Y H:i',strtotime($sh['received_at'])):'–')?></td></tr><?php endforeach;?><?php if(!$shipments):?><tr><td colspan="6">Keine Versandvorgänge.</td></tr><?php endif;?></tbody></table></div>

    <div class="grid two" style="margin-top:18px">
      <section class="panel"><h2>Nachweise</h2><div class="table-wrap"><table><thead><tr><th>Zeit</th><th>Auftrag</th><th>Typ</th><th>Tag/Fenster</th><th>Status</th><th></th></tr></thead><tbody><?php foreach($sellerEvidence as $ev):?><tr><td><?=e(date('d.m.Y H:i',strtotime($ev['created_at'])))?></td><td><a href="<?=e(url('/admin/auftrag/'.$ev['order_no']))?>"><?=e($ev['order_no'])?></a></td><td><?=e($ev['evidence_type'])?></td><td><?=e(($ev['day_no']?'Tag '.$ev['day_no']:'').($ev['window_key']?' · '.$ev['window_key']:''))?></td><td><?=e($ev['status'])?></td><td><a href="<?=e(url('/datei/'.$ev['id']))?>" target="_blank">Ansehen</a></td></tr><?php endforeach;?><?php if(!$sellerEvidence):?><tr><td colspan="6">Keine Nachweise.</td></tr><?php endif;?></tbody></table></div></section>

      <section class="panel"><h2>Verstöße</h2><div class="table-wrap"><table><thead><tr><th>Zeit</th><th>Auftrag</th><th>Typ</th><th>Status</th><th>Grund</th></tr></thead><tbody><?php foreach($sellerViolations as $v):?><tr><td><?=e(date('d.m.Y H:i',strtotime($v['created_at'])))?></td><td><a href="<?=e(url('/admin/auftrag/'.$v['order_no']))?>"><?=e($v['order_no'])?></a></td><td><?=e($v['violation_type'])?></td><td><?=e($v['status'])?></td><td><?=e($v['reason']??'')?></td></tr><?php endforeach;?><?php if(!$sellerViolations):?><tr><td colspan="5">Keine Verstöße.</td></tr><?php endif;?></tbody></table></div></section>
    </div>

    <h2>Chronologische Aktivität</h2><div class="table-wrap"><table><thead><tr><th>Zeit</th><th>Ereignis</th><th>Auftrag</th><th>Details</th></tr></thead><tbody><?php foreach($sellerEvents as $event):?><tr><td><?=e(date('d.m.Y H:i',strtotime($event['created_at'])))?></td><td><?=e($event['event_type'])?></td><td><?=e($event['order_id']?:'–')?></td><td><code><?=e($event['payload_json']??'')?></code></td></tr><?php endforeach;?><?php if(!$sellerEvents):?><tr><td colspan="4">Keine protokollierten Ereignisse.</td></tr><?php endif;?></tbody></table></div>
