...
if ($path==='/admin/systemausfaelle' && $method==='GET') {
    require_admin();
    $rows=db()->query("SELECT so.*,(SELECT COUNT(*) FROM outage_order_impacts oi WHERE oi.outage_id=so.id) affected_orders FROM system_outages so ORDER BY so.starts_at DESC")->fetchAll();
    ob_start();?><div class="dashboard-head"><div><div class="eyebrow">Administration</div><h1>Systemausfälle</h1><p class="meta">Dokumentierte technische Plattformausfälle verschieben betroffene Fristen. Rein aus dem Ausfall entstandene vorläufige Verstöße werden verworfen.</p></div></div>
    <form class="panel" method="post"><?=csrf_field()?><div class="form-grid"><label>Beginn<input type="datetime-local" name="starts_at" required></label><label>Ende<input type="datetime-local" name="ends_at" required></label></div><label>Grund / technische Störung<textarea name="reason" required></textarea></label><button class="btn">Ausfall dokumentieren & Fristen verschieben</button></form>
    <h2>Historie</h2><div class="table-wrap"><table><thead><tr><th>Beginn</th><th>Ende</th><th>Dauer</th><th>Betroffene Aufträge</th><th>Grund</th></tr></thead><tbody><?php foreach($rows as $r):?><tr><td><?=e(date('d.m.Y H:i',strtotime($r['starts_at'])))?></td><td><?=e(date('d.m.Y H:i',strtotime($r['ends_at'])))?></td><td><?=e(round($r['duration_seconds']/60))?> Min.</td><td><?=e($r['affected_orders'])?></td><td><?=e($r['reason'])?></td></tr><?php endforeach;?></tbody></table></div>
    <?php if(!$rows):?><div class="empty">Noch keine Systemausfälle dokumentiert.</div><?php endif;?>
    <?php render('Systemausfälle',ob_get_clean());exit;
}

if ($path==='/admin/systemausfaelle' && $method==='POST') {
    require_admin();
    $tz=new DateTimeZone((string)app_config('app.timezone','Europe/Berlin'));
    try{
        ...
        flash('error','Beginn oder Ende ist ungültig.');redirect('/admin/systemausfaelle');
    }
    if($end<=$start){flash('error','Das Ende muss nach dem Beginn liegen.');redirect('/admin/systemausfaelle');}
    $reason=post('reason');if($reason===''){flash('error','Bitte den Grund des Ausfalls dokumentieren.');redirect('/admin/systemausfaelle');}
    $seconds=$end->getTimestamp()-$start->getTimestamp();
    if($seconds>31*86400){flash('error','Ein einzelner Ausfallzeitraum darf maximal 31 Tage umfassen.');redirect('/admin/systemausfaelle');}

    $startSql=$start->format('Y-m-d H:i:s');$endSql=$end->format('Y-m-d H:i:s');
    $activeOrders=db()->query("SELECT id,order_no,seller_id FROM orders WHERE archived_at IS NULL AND status IN('precheck','running','shipping','review','payout')")->fetchAll();

    db()->beginTransaction();
    try{
        db()->prepare("INSERT INTO system_outages(starts_at,ends_at,duration_seconds,reason) VALUES(?,?,?,?)")->execute([$startSql,$endSql,$seconds,$reason]);
        $outageId=(int)db()->lastInsertId();

        foreach($activeOrders as $o){
            $orderId=(int)$o['id'];

            // Quellen merken, die während des Ausfalls bereits als versäumt erkannt worden sein könnten.
            $wq=db()->prepare("SELECT id FROM evidence_windows WHERE order_id=? AND starts_at<=? AND COALESCE(grace_ends_at,ends_at)>=?");
            $wq->execute([$orderId,$endSql,$startSql]);$windowIds=array_map('intval',array_column($wq->fetchAll(),'id'));
            $sq=db()->prepare("SELECT id FROM spontaneous_requests WHERE order_id=? AND due_at<=? AND COALESCE(grace_ends_at,due_at)>=?");
            $sq->execute([$orderId,$endSql,$startSql]);$spontIds=array_map('intval',array_column($sq->fetchAll(),'id'));
            $tq=db()->prepare("SELECT id FROM order_tasks WHERE order_id=? AND due_at IS NOT NULL AND due_at BETWEEN ? AND ?");
            ...
            db()->exec("UPDATE order_shipping_steps SET due_at=DATE_ADD(due_at,INTERVAL {$int} SECOND) WHERE order_id={$orderId} AND due_at IS NOT NULL AND due_at>=".$pdoQuote);

            $sourcePatterns=[];
            foreach($windowIds as $id)$sourcePatterns[]='window-'.$id.'-missing-%';
            foreach($spontIds as $id)$sourcePatterns[]='spontaneous-'.$id.'-missing-%';
            foreach($taskIds as $id)$sourcePatterns[]='task-'.$id.'-missed';
            foreach($sourcePatterns as $pattern){
                $vq=db()->prepare("SELECT id FROM violations WHERE order_id=? AND status IN('open','reviewed') AND source_key LIKE ?");
                $vq->execute([$orderId,$pattern]);
                foreach($vq->fetchAll() as $vr){
                    db()->prepare("UPDATE violations SET status='discarded',reviewed_at=NOW(),reason=CONCAT(COALESCE(reason,''),' [Systemausfall: automatisch verworfen]') WHERE id=?")->execute([$vr['id']]);
                    db()->prepare("UPDATE extra_days SET status='cancelled' WHERE source_type='violation' AND source_id=? AND status='provisional'")->execute([$vr['id']]);
                }
            }

            db()->prepare("INSERT IGNORE INTO outage_order_impacts(outage_id,order_id,shifted_seconds) VALUES(?,?,?)")->execute([$outageId,$orderId,$seconds]);
            db()->prepare("INSERT INTO chat_messages(order_id,sender_type,message) VALUES(?,'system',?)")
              ->execute([$orderId,'Systemausfall dokumentiert: '.$start->format('d.m.Y H:i').' bis '.$end->format('d.m.Y H:i').'. Betroffene Fristen wurden um '.round($seconds/60).' Minuten verschoben.']);
            log_event('system.outage',(int)$o['seller_id'],$orderId,['outage_id'=>$outageId,'starts_at'=>$start->format(DATE_ATOM),'ends_at'=>$end->format(DATE_ATOM),'shift_seconds'=>$seconds]);
        }

        // Auch noch nicht angenommene individuelle Angebote sind während eines Plattformausfalls nicht erreichbar.
        $int=(int)$seconds;$quoted=db()->quote($startSql);
        db()->exec("UPDATE offer_assignments SET acceptance_deadline=DATE_ADD(acceptance_deadline,INTERVAL {$int} SECOND),updated_at=NOW() WHERE status='assigned' AND acceptance_deadline>={$quoted}");

        db()->commit();
    }catch(Throwable $e){db()->rollBack();throw $e;}

    foreach($activeOrders as $o){
        notify_seller((int)$o['seller_id'],'system.outage','Technischer Plattformausfall dokumentiert','Betroffene Fristen in Auftrag '.$o['order_no'].' wurden um '.round($seconds/60).' Minuten verschoben.','/auftrag/'.$o['order_no'],'outage-'.$outageId.'-order-'.$o['id'],true);
    }
