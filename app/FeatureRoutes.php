...
if (preg_match('#^/admin/angebot/(\\d+)/duplizieren$#',$path,$m) && $method==='POST') {
    ...
    db()->beginTransaction();try{
        db()->prepare("INSERT INTO offers(category_id,title,slug,description,compensation,duration_days,fulfillment_type,evidence_rules_json,shipping_rules_json,shipping_address_id,shipping_cost_mode,shipping_allowance,preferred_carrier,shipping_window_hours,status,current_version) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,'draft',1)")
            ->execute([$o['category_id'],$title,$slug,$o['description'],$o['compensation'],$o['duration_days'],$o['fulfillment_type'],$o['evidence_rules_json'],$o['shipping_rules_json'],$o['shipping_address_id'],$o['shipping_cost_mode'],$o['shipping_allowance'],$o['preferred_carrier'],$o['shipping_window_hours']]);
