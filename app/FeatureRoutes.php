...
if (preg_match('#^/auftrag/(\d{8})/versand$#',$path,$m)&&$method==='GET') {
    ...
    <?php if($o['status']==='shipping' || $ship):?><section class="panel"><h2>Versandinformationen</h2>
      ...
      <?php if(!empty($o['shipping_due_at'])):?><p><span class="meta">Gesamt-Versandfrist</span><br><strong><?=e(date('d.m.Y H:i',strtotime($o['shipping_due_at'])))?> Uhr</strong><?php if((int)setting_value('grace_minutes','60')>0):?><br><small>Nachfrist: <?=e((int)setting_value('grace_minutes','60'))?> Minuten</small><?php endif;?></p><?php elseif(!empty($shippingSnapshot['window_hours'])):?><p><span class="meta">Gesamt-Versandfrist</span><br><?=e((int)$shippingSnapshot['window_hours'])?> Stunden ab Freischaltung</p><?php endif;?>
...
<label>Gesamt-Versandfrist ab Freischaltung (Stunden)<input type="number" min="1" name="shipping_window_hours" value="<?=e($o['shipping_window_hours']??'')?>" placeholder="leer = keine Gesamtfrist"></label>
...
if (preg_match('#^/admin/angebot/(\\d+)$#',$path,$m)&&$method==='POST') {
    ...
    $snap=['title'=>post('title'),'category_id'=>(int)post('category_id'),'description'=>post('description'),'compensation'=>$comp,'duration_days'=>$days,'fulfillment_type'=>post('fulfillment_type'),'status'=>post('status'),'evidence_rules'=>$rules,'digital_rules'=>$digitalRules,'shipping'=>['address_id'=>$shippingAddressId,'cost_mode'=>$shippingCostMode,'allowance'=>$shippingAllowance,'preferred_carrier'=>$preferredCarrier,'window_hours'=>$shippingWindowHours,'instructions'=>$shippingRules['instructions']],'components'=>offer_component_definitions($o)];
    db()->beginTransaction();try{
      db()->prepare("UPDATE offers SET category_id=?,title=?,description=?,compensation=?,duration_days=?,fulfillment_type=?,evidence_rules_json=?,digital_rules_json=?,shipping_rules_json=?,shipping_address_id=?,shipping_cost_mode=?,shipping_allowance=?,preferred_carrier=?,shipping_window_hours=?,status=?,current_version=?,updated_at=NOW() WHERE id=?")->execute([$snap['category_id'],$snap['title'],$snap['description'],$comp,$days,$snap['fulfillment_type'],$rulesJson,$digitalRulesJson,$shippingRulesJson,$shippingAddressId,$shippingCostMode,$shippingAllowance,$preferredCarrier,$shippingWindowHours,$snap['status'],$newVersion,$o['id']]);
...
if (preg_match('#^/admin/angebot/(\\d+)/duplizieren$#',$path,$m) && $method==='POST') {
    ...
    db()->beginTransaction();try{
        db()->prepare("INSERT INTO offers(category_id,title,slug,description,compensation,duration_days,fulfillment_type,evidence_rules_json,digital_rules_json,shipping_rules_json,shipping_address_id,shipping_cost_mode,shipping_allowance,preferred_carrier,shipping_window_hours,status,visibility,current_version) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'draft',?,1)")
            ->execute([$o['category_id'],$title,$slug,$o['description'],$o['compensation'],$o['duration_days'],$o['fulfillment_type'],$o['evidence_rules_json'],$o['digital_rules_json'],$o['shipping_rules_json'],$o['shipping_address_id'],$o['shipping_cost_mode'],$o['shipping_allowance'],$o['preferred_carrier'],$o['shipping_window_hours'],$o['visibility']]);
        $id=(int)db()->lastInsertId();
        $snap=json_encode(['category_id'=>(int)$o['category_id'],'title'=>$title,'description'=>$o['description'],'compensation'=>(float)$o['compensation'],'duration_days'=>$o['duration_days'],'fulfillment_type'=>$o['fulfillment_type'],'status'=>'draft','visibility'=>$o['visibility'],'evidence_rules'=>json_decode($o['evidence_rules_json']?:'{}',true)?:[],'digital_rules'=>offer_digital_rules($o),'shipping'=>build_shipping_snapshot($o)],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
        ...
        $components=db()->prepare("INSERT INTO offer_components(offer_id,category_id,title,component_type,compensation,duration_days,required,sort_order,active) SELECT ?,category_id,title,component_type,compensation,duration_days,required,sort_order,active FROM offer_components WHERE offer_id=?");$components->execute([$id,$o['id']]);
        db()->commit();
...
if ($path==='/individuelle-angebote' && $method==='GET') {
    ...
    <?php foreach($rows as $r): $rRules=offer_evidence_rules($r);$rDaily=array_sum($rRules['daily']);$rTasks=offer_task_plan_summary((int)$r['offer_id'],$r['duration_days']!==null?(int)$r['duration_days']:1);$rShipping=build_shipping_snapshot($r);$rAllowance=($rShipping['cost_mode']??'seller')==='fixed'?(float)($rShipping['allowance']??0):0.0;$rComponentExtra=offer_component_extra_total($r);$rTotal=(float)$r['compensation']+$rComponentExtra+(float)$rTasks['compensation']+$rAllowance;$rOptQ=db()->prepare("SELECT * FROM offer_options WHERE offer_id=? AND active=1 ORDER BY id");$rOptQ->execute([$r['offer_id']]);$rOptions=$rOptQ->fetchAll();$rHasDigital=in_array($r['fulfillment_type'],['digital','mixed'],true);if(!$rHasDigital){$rDc=db()->prepare("SELECT COUNT(*) FROM offer_components WHERE offer_id=? AND component_type='digital'");$rDc->execute([$r['offer_id']]);$rHasDigital=(int)$rDc->fetchColumn()>0;}$rDigitalRules=$rHasDigital?offer_digital_rules($r):null;?><article class="card"><span class="badge"><?=e($r['assignment_status'])?></span><h2><?=e($r['title'])?></h2><p><?=nl2br(e($r['description']))?></p><div class="price"><?=money($rTotal)?></div><p class="meta"><?=e($r['duration_days']?$r['duration_days'].' Tage':ucfirst($r['fulfillment_type']))?> · Annahmefrist <?=e(date('d.m.Y H:i',strtotime($r['acceptance_deadline'])))?></p>
      <div class="timeline"><div>Vorabnachweise: <strong><?=e($rRules['precheck_required_count'])?></strong></div><div>Regel-Nachweise: <strong><?=e($rDaily)?> pro Tag</strong></div><div>Geplante Zusatzaufgaben: <strong><?=e($rTasks['executions'])?></strong></div><div>Versand-/Endschritte: <strong><?=e(count($rShipping['steps']??[]))?></strong></div><?php if($rComponentExtra>0):?><div>Kombi-/Zusatzbestandteile: <strong>+<?=money($rComponentExtra)?></strong></div><?php endif;?><?php if($rTasks['compensation']>0):?><div>Geplante Aufgaben: <strong>+<?=money($rTasks['compensation'])?></strong></div><?php endif;?><?php if($rAllowance>0):?><div>Versandzuschuss: <strong>+<?=money($rAllowance)?></strong></div><?php endif;?><?php if($rHasDigital):?><div>Digitale Abgabe: <strong><?=e(digital_rules_summary($rDigitalRules))?></strong></div><div>Erstabgabe: <strong>innerhalb <?=e($rDigitalRules['deadline']['hours_after_acceptance'])?> Stunden nach Annahme</strong></div><?php endif;?></div>
      <?php if($rOptions):?><h3>Wählbare Zusatzoptionen</h3><div class="timeline"><?php foreach($rOptions as $opt):?><div><?=e($opt['label'])?> · <?=$opt['price']>0?('+'.money($opt['price'])):'kostenlos'?></div><?php endforeach;?></div><?php endif;?>
      ...
      <?php if($r['assignment_status']==='assigned'&&strtotime($r['acceptance_deadline'])>time()):?><div class="grid two"><form method="post" action="<?=e(url('/individuelle-angebote/'.$r['assignment_id'].'/annehmen'))?>"><?=csrf_field()?>
        <?php foreach($rOptions as $opt):?><label style="display:flex;gap:10px;align-items:flex-start"><input type="checkbox" style="width:auto;margin-top:5px" name="option_ids[]" value="<?=e($opt['id'])?>"><span><?=e($opt['label'])?> · <?=$opt['price']>0?('+'.money($opt['price'])):'kostenlos'?></span></label><?php endforeach;?>
...
if (preg_match('#^/individuelle-angebote/(\d+)/(annehmen|ablehnen)$#',$path,$m) && $method==='POST') {
    ...
    $requestedOptions=array_values(array_unique(array_map('intval',(array)($_POST['option_ids']??[]))));
    $selectedOptions=[];$optionsTotal=0.0;
    if($requestedOptions){
        $ph=implode(',',array_fill(0,count($requestedOptions),'?'));
        $args=array_merge([$a['offer_id']],$requestedOptions);
        $oq=db()->prepare("SELECT * FROM offer_options WHERE offer_id=? AND active=1 AND id IN ($ph) ORDER BY id");
        $oq->execute($args);$selectedOptions=$oq->fetchAll();
        if(count($selectedOptions)!==count($requestedOptions)){flash('error','Mindestens eine ausgewählte Zusatzoption ist nicht mehr verfügbar.');redirect('/individuelle-angebote');}
    ...
    $pureDigital=offer_is_pure_digital($a);
    ...
    $total=(float)$a['compensation']+$componentExtra+$optionsTotal+$plannedTaskCompensation+$shippingAllowance;
    $no=order_number();
    db()->beginTransaction();
    try{
        ...
        $confirmationPayload=[
          ...
          'base_compensation'=>(float)$a['compensation'],'component_extra_compensation'=>$componentExtra,'options_total'=>$optionsTotal,'selected_options'=>array_map(fn($opt)=>['id'=>(int)$opt['id'],'label'=>$opt['label'],'price'=>(float)$opt['price']],$selectedOptions),
          'evidence_rules'=>$acceptanceRules,'precheck_photos'=>$pureDigital?0:(int)$acceptanceRules['precheck_required_count'],'daily_photos_per_day'=>$pureDigital?0:array_sum($acceptanceRules['daily']),
        ...
        log_event('private_offer.accepted',(int)$s['id'],$oid,['assignment_id'=>(int)$a['assignment_id'],'option_ids'=>$requestedOptions,'options_total'=>$optionsTotal,'shipping_allowance'=>$shippingAllowance,'component_extra_compensation'=>$componentExtra,'planned_task_compensation'=>$plannedTaskCompensation,'planned_task_executions'=>$taskSummary['executions'],'blocked_category_ids'=>$blockedCategories,'digital_due_at'=>$digitalDueAt,'pure_digital'=>$pureDigital,'total'=>$total]);
        db()->commit();
    }catch(Throwable $e){db()->rollBack();throw $e;}
    ...
    send_app_mail(
      ...
      ($optionsTotal>0?'Zusatzoptionen: <strong>+'.money($optionsTotal).'</strong><br>':'').
      ...
      '<p>Dauer: '.($a['duration_days']!==null?e($a['duration_days']).' Tage':'individueller Umfang').'<br>'.($pureDigital?'Digitale Durchführung startet sofort.<br>':'Vorabnachweise: '.e($acceptanceRules['precheck_required_count']).' Foto(s)<br>Regel-Nachweise pro Tag: '.e(array_sum($acceptanceRules['daily'])).' Foto(s)<br>').'Geplante Zusatzaufgaben: '.e($taskSummary['executions']).'<br>'.($pureDigital?'Digitale Abgabe innerhalb der Plattform':'Versand: '.e($shippingText)).'</p>'.
    ...
    flash('success','Individuelles Angebot angenommen. Auftrag '.$no.' wurde erstellt. Gesamtwert: '.money($total).'. Eine Auftragsbestätigung wurde per E-Mail versendet.'.($pureDigital?' Die digitale Durchführung ist sofort aktiv.':''));redirect($pureDigital?'/auftrag/'.$no.'/digital':'/auftrag/'.$no);
}
