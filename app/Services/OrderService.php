...
class OrderService {
    ...
    public function create(User $user, Offer $offer, array $optionIds): Order {
        return DB::transaction(function () use ($user,$offer,$optionIds) {
            ...
            abort_if(count(array_unique($optionIds)) !== $options->count(), 422, 'Ungültige Zusatzoption.');
            ...
            foreach ($options as $option) {
                $order->options()->create(['offer_option_id'=>$option->id,'name'=>$option->name,'price_delta'=>$option->price_delta,'snapshot'=>$option->toArray()]);
            }
