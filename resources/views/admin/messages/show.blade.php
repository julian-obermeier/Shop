...
<div>
<span class="eyebrow">Nachrichtenverwaltung</span>
<h1>{{ $conversation->subject }}</h1>
<p>{{ $conversation->user->first_name }} {{ $conversation->user->last_name }} · Auftrag #{{ $conversation->order->order_number }}</p>
</div>
@if($writeLocked)<span class="status pending">NUR LESEN</span>@endif
</div>

<div class="panel">
@foreach($conversation->messages as $message)
<div style="padding:14px 0;border-bottom:1px solid var(--line)">
<strong>{{ $message->user->isAdmin() ? 'Admin' : $message->user->first_name }}</strong>
<small class="muted"> · {{ $message->created_at->format('d.m.Y H:i') }}@if($message->read_at) · gelesen@endif</small>
...
@else
<div class="notice" style="margin-top:16px">Der Auftrag ist seit mehr als 7 Tagen abgeschlossen. Der Nachrichtenverlauf bleibt sichtbar, ist aber endgültig schreibgeschützt.</div>
@endif
@endsection
