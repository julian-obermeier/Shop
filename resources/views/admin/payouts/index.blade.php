...
@if(!in_array($payout->status,['paid','rejected','failed','cancelled'],true))
<form method="post" action="{{ route('admin.payouts.update',$payout) }}" class="stack-form">@csrf
<select name="status"><option value="review">In Prüfung</option><option value="approved">Freigegeben</option><option value="paid">Ausgezahlt</option><option value="rejected">Abgelehnt</option><option value="failed">Fehlgeschlagen</option><option value="cancelled">Storniert</option></select>
<textarea name="admin_note" rows="2" placeholder="Notiz"></textarea>
