...
@section('content')
<div class="page-head"><span class="eyebrow">Konto</span><h1>Profil & Kontostatus</h1><p>Stammdaten, aktive Einschränkungen und Verwarnungen.</p></div>
...
<aside>
<div class="panel"><h2>Kontostatus</h2><dl class="meta-list"><div><dt>Status</dt><dd>{{ strtoupper($user->status) }}</dd></div><div><dt>Verifizierung</dt><dd>{{ $user->verified_at?'abgeschlossen':'offen' }}</dd></div></dl></div>
<div class="panel"><h2>Aktive Einschränkungen</h2>@forelse($user->restrictions->where('active',true) as $item)<div class="notice"><strong>{{ strtoupper($item->type) }}</strong><br>{{ $item->reason }}@if($item->ends_at)<br><small>bis {{ $item->ends_at->format('d.m.Y H:i') }}</small>@endif</div>@empty<p class="muted">Keine aktiven Einschränkungen.</p>@endforelse</div>
<div class="panel"><h2>Verwarnungen</h2>@forelse($user->warnings->sortByDesc('created_at') as $warning)<div style="padding:10px 0;border-bottom:1px solid var(--line)"><strong>{{ $warning->title }}</strong><p>{{ $warning->reason }}</p><small class="muted">{{ $warning->created_at->format('d.m.Y H:i') }}</small></div>@empty<p class="muted">Keine Verwarnungen.</p>@endforelse</div>
</aside></div>
@endsection
